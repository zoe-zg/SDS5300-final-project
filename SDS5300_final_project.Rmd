...
f1_data <- f1_results %>%  
  ...
  dplyr::select(raceId, year, round, driverId, circuitId, grid, positionOrder, points, 
...
plot(f1_clean$grid, f1_clean$positionOrder,
     ...
     pch = 19, col = rgb(0,0,0,0.2)) # Use alpha for overlapping points
...
f1_clean %>%
  ggplot(aes(x = circuitId, y = fastestLapSpeed, fill = circuitId)) +
